...
public class ImSystemMessageController extends BaseController {
    ...
    public void export(ImSystemMessageBo bo, HttpServletResponse response) {
        List<ImSystemMessageVo> list = systemMessageService.queryList(bo);
    ...
    @SaCheckPermission("im:systemMessage:push")
    @Log(title = "系统消息", businessType = BusinessType.UPDATE)
    @PostMapping("/push/{id}")
    public R<Void> push(@NotNull(message = "主键不能为空") @PathVariable Long id) {
        return toAjax(systemMessageService.push(id));
    }
